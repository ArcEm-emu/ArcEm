 ...
 */
static ARMword RowBuffer[PDD_MonitorWidth/4] ALIGN(8);

typedef struct {
	u8 *dst, *src;
	unsigned int width; /* Width of area being updated */
} PDD_Row;


static void PDD_Name(Host_ChangeMode)(ARMul_State *state,int width,int height,int depth,int hz);

static inline void PDD_Name(Host_SetPaletteEntry)(ARMul_State *state,int i,unsigned int phys)
{
	BG_PALETTE[i] = ConvertPhysToColour(phys);
}

static inline void PDD_Name(Host_SetBorderColour)(ARMul_State *state,unsigned int phys)
{
	// TODO: Implement this!
}
...
{
	PDD_Row drow;
	drow.dst = ((u8 *)bgGetGfxPtr(background)) + (row * PDD_MonitorWidth) + offset;
	drow.src = ((u8 *)RowBuffer);
	drow.width = 0;
	*alignment = 0;
	return drow;
}
...
static inline void PDD_Name(Host_EndRow)(ARMul_State *state,PDD_Row *row)
{
	...
	if (row->width > 0) {
		DC_FlushRange(row->src, row->width);
		dmaCopyWords(3, row->src, row->dst, row->width);
	}
}
...
{
	*outoffset = 0;
	return (ARMword *)(row->src + row->width);
}
...
static inline void PDD_Name(Host_AdvanceRow)(ARMul_State *state,PDD_Row *row,unsigned int count)
{
	if (row->width > 0) {
		while (dmaBusy(3));
		DC_FlushRange(row->src, row->width);
		dmaCopyWordsAsynch(3, row->src, row->dst, row->width);
		row->dst += row->width;
		row->src += row->width;
		row->width = 0;
	}
